...
sys.path.append(os.path.join(os.path.dirname(__file__), 'bilstm'))

from transformers import T5Tokenizer

from mt5.infer import summarize
from bilstm.infer import sentiment,LABEL_MAP

def token_count(text,model_name):
    return len(T5Tokenizer.from_pretrained(model_name,legacy=False).tokenize(str(text)))

def main():
    parser = argparse.ArgumentParser("Inference mT5 BiLSTM")
    ...
    # BiLSTM
    parser.add_argument("--bilstm_model_dir", type=str, default="models/bilstm/bilstm_best.pt")
    ...
    pred_id =sentiment(text,args.bilstm_model_dir,args.device,args.vocab_json,args.embeddings_path,args.max_len)
