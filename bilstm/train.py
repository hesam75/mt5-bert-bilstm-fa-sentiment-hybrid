...
from model import BiLSTMClassifier, load_embeddings, count_params
...
def evaluate_model(model: nn.Module, loader, device: torch.device) -> Tuple[dict, np.ndarray, np.ndarray]:
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            logits = model(batch["input_ids"], batch["lengths"], batch["attention_mask"])
...
def plot_lr(lr_history: list, out_dir: str):
    if not lr_history:
        return
    os.makedirs(out_dir, exist_ok=True)
    plt.figure()
...
def train_one_epoch(model, loader, optimizer, criterion, device, scaler=None,
                    grad_accum_steps=1, max_norm=None, scheduler=None, step_scheduler_per_batch=False, epoch:int=None, epochs:int=None, show_progress:bool=True):
    model.train()
    running_loss = 0.0
    running_correct = 0
    n_samples = 0
    ...
    iterator = tqdm(loader, total=len(loader), desc=desc, ncols=100, leave=False)
    ...
    for step, batch in enumerate(iterator, start=1):
        ...
        if show_progress:
            ...
            iterator.set_postfix({"loss": f"{avg_loss:.4f}", "acc": f"{avg_acc:.3f}", "lr": f"{lr_now:.2e}"})
...
# ------------------------- Main -------------------------
def main():
    parser = argparse.ArgumentParser(description="Step 5: Train BiLSTM")
    ...
    parser.add_argument("--chart_dir", type=str, default="chart/bilstm")
    parser.add_argument("--logs_dir", type=str, default="logs/bilstm")
    ...
    if args.loss == "ce":
        weight_vec = torch.tensor([class_weights[i] for i in range(num_classes)], dtype=torch.float32).to(device)
        criterion = nn.CrossEntropyLoss(weight=weight_vec, reduction="mean")
    else:
        criterion = FocalLoss(num_classes=num_classes, gamma=args.gamma, alpha=class_weights)
    ...
    best_path = os.path.join(args.out_dir, "bilstm_best.pt")
