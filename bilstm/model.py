...
class BiLSTMClassifier(nn.Module):
    ...
    @staticmethod
    def _select_last_valid(h: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        B, T, H2 = h.size()
        idx = (lengths - 1).clamp(min=0).view(B, 1, 1).expand(B, 1, H2)
        gathered = h.gather(dim=1, index=idx).squeeze(1) 
        return gathered

    def forward(self, input_ids: torch.Tensor, lengths: torch.Tensor, attention_mask: torch.Tensor):
...
# ---------------------- Main ----------------------
def main():
    parser = argparse.ArgumentParser(description="Step 4: BiLSTM Model")
