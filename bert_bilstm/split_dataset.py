...
def read_dataset(csv_path: str) -> pd.DataFrame:
    ...
    after = len(df)
    if after < before:
        print(f"{before - after} rows were deleted due to NaN.")
    ...
    try:
        ...
        raise ValueError("The 'score' column must be convertible to int.") from e

    valid_set = {0, 1, 2}
    bad_mask = ~df["score"].isin(valid_set)
    if bad_mask.any():
        bad_vals = df.loc[bad_mask, "score"].unique().tolist()
        raise ValueError(f"Invalid values in 'score': {bad_vals}; only {valid_set} is allowed.")
...
def verify_sizes(original_df: pd.DataFrame, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame):
    ...
    assert n_total == (n_train + n_val + n_test), "The sum of the sizes is not equal to the entire dataset!"
...
def ensure_each_class_present(df: pd.DataFrame, name: str):
    ...
    if missing:
        raise AssertionError(f"{name} is missing these classes: {missing}")
...
def save_splits(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, out_dir: str = "."):
    ...
    for name, part in [("train", train_df), ("val", val_df), ("test", test_df)]:
        ...
        print(f"[OK] File saved: {out_path}  ({len(to_save):,} rows)")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="datasets/bert-bilstm")
    ...
    # Quick guide to calculating class weight from TRAIN (for next steps)
